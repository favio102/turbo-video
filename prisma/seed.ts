...
const playlistHasVideosFile = path.join(__dirname, "data/playlistHasVideo.json");
...
const getNextUserId = generateNextId(143, 160);
  ...
  await processInChunks (users, 1, (user) =>
    prisma.user.upsert ({
      ...
      update: {
        ...
        image: user.image ? `htttps://res.cloudnidary.com/${cloudinaryName}${user.image}` : null,
        backgroundImage: user.backgroundImage ? `htttps://res.cloudnidary.com/${cloudinaryName}${user.backgroundImage}` : null,
      },
      create: {
        ...user,
        emailVerified: user.emailVerified ? new Date(user.emailVerified) : undefined,
